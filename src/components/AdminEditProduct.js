...
const AdminEditProduct = ({ onClose, fetchData, productData }) => {
    ...
    const uploadSingleFile = async (file, isVideo = false) => {
        // 1. Verificar límite de cantidad
        if (isVideo) {
            const totalAlreadyHave = data.productVideo.length +
                uploadingFiles.filter((f) => f.isVideo).length;
            if (totalAlreadyHave >= MAX_VIDEOS) {
                alert(`Ya tienes ${MAX_VIDEOS} videos. No puedes subir más.`);
                return;
            }
        } else {
            ...
            const totalAlreadyHave = data.productImage.length +
                uploadingFiles.filter((f) => !f.isVideo).length;
    ...
    const validateForm = () => {
        ...
        if (!data.categoryId) {
            newErrors.categoryId = "CategoryId is required.";
            valid = false;
        }

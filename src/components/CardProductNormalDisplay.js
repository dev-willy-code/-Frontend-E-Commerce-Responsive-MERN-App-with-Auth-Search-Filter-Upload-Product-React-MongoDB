...
const CardProductNormalDisplay = ({ category, heading }) => { //props
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            console.log("ProductsByCategory: ", categoryProduct);
...

                        {
                            loading ? (
                                ...
                                data.map((product, index) => {
                                    return (
                                        <Link to={"/product/" + product?._id} className='bg-white rounded-sm shadow-md' onClick={scrollTop}>
                                            ...
                                            Si es más pequeña que el contenedor, se comporta como object-none (no escala). */}
                                                <img src={product?.productImage[0]} className='object-scale-down w-fit h-full hover:scale-105 transition-all mix-blend-multiply dark:mix-blend-normal' />
                                            </div>
                                            <div className='p-4 grid gap-3 bg-white dark:bg-slate-700'>
                                                ...
                                                <div className='flex gap-3'>
                                                    ...
                                                    <p className='text-slate-500 dark:text-slate-100 line-through'>{displayINRCurrency(product?.price)}</p>
                                                </div>
...
export default CardProductNormalDisplay
